...
(function() {
    ...
    function drawSun() {
        // 大范围金色光辉（洒向整个板块）- 再降低强度
        const sunLightRadius = Math.max(w, h) * 0.5;
        const sunLightGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, sunLightRadius);
        sunLightGrad.addColorStop(0, 'rgba(255, 200, 60, 0.018)');
        sunLightGrad.addColorStop(0.3, 'rgba(255, 160, 20, 0.01)');
        sunLightGrad.addColorStop(0.6, 'rgba(255, 120, 0, 0.005)');
        sunLightGrad.addColorStop(1, 'rgba(255, 80, 0, 0)');
        ctx.fillStyle = sunLightGrad;
        ctx.beginPath();
        ctx.arc(cx, cy, sunLightRadius, 0, Math.PI * 2);
        ctx.fill();
